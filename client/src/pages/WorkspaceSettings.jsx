	...
	return (
		<div
			...
			style={{
				...
				background: danger ? "var(--danger-muted)" : "var(--bg-surface)",
			}}
		>
			<h3 style={{ fontSize: 16, fontWeight: 600, color: danger ? "var(--danger)" : "var(--text-primary)", margin: 0 }}>{title}</h3>
			{subtitle && (
				<p style={{ fontSize: 13, color: danger ? "rgba(248,113,113,0.85)" : "var(--text-secondary)", margin: "4px 0 0" }}>{subtitle}</p>
			)}
...
export default function WorkspaceSettings() {
	...
	const canManageInvites = ["admin", "owner"].includes(workspace?.currentUserRole);
	...
	const loadData = async () => {
		...
		try {
			...
			setGeneralForm({ name: workspaceData?.name || "", description: workspaceData?.description || "" });
			const canInvite = ["admin", "owner"].includes(workspaceData?.currentUserRole);
			if (canInvite) {
				try {
					const invitesRes = await api.get(`/workspaces/${workspaceId}/invites`);
					setPendingInvites(invitesRes.data.invites || []);
				} catch (inviteErr) {
					setInviteError(inviteErr.response?.data?.message || "Failed to load pending invites");
					setPendingInvites([]);
				}
			} else {
				setPendingInvites([]);
	...
	useEffect(() => {
		if (!workspaceId || !canManageInvites) return undefined;
	...
	}, [query, workspaceId, canManageInvites]);
	...
	const handleInvite = async (email) => {
		if (!email) return;
		if (!canManageInvites) {
			setInviteError("Requires admin role or higher");
			return;
		}
	...
	if (status === "loading") {
		return (
			...
			>
				<div
					className="skeleton"
					style={{ height: 28, width: 240, marginBottom: 12 }}
				/>
				<div
					className="skeleton"
	...
	return (
		...
		>
			...
			{activeTab === "general" && (
				<SettingsCard
					title="General"
					subtitle="Rename this workspace and keep its purpose visible to members."
				>
					<form
			...
			{activeTab === "members" && (
				<div
					style={{
						display: "grid",
						gap: 18,
						gridTemplateColumns: canManageInvites ? "minmax(0, 1.15fr) minmax(280px, 0.85fr)" : "minmax(0, 1fr)",
					}}
				>
					<div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
						{canManageInvites ? (
							<SettingsCard
								...
								subtitle="Search an existing account or send an email invite."
							>
								<div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
									<div style={{ position: "relative", minWidth: 240, flex: 1 }}>
										...
										{(query.trim() || isSearching) && (
											<div
												className="dropdown"
												style={{ left: 0, right: 0, top: "calc(100% + 8px)", minWidth: "100%" }}
											>
												{isSearching && (
													<p style={{ padding: "10px 12px", margin: 0, fontSize: 12, color: "var(--text-muted)" }}>
														Searching...
													</p>
												)}
												{!isSearching &&
													results.map((result) => (
														...
														>
															<span>
																...
																<span style={{ fontSize: 12 }}>{result.email}</span>
															</span>
												...
												{!isSearching && !results.length && emailFallbackVisible && (
													...
													>
														<span>
															<strong style={{ display: "block", color: "var(--text-primary)" }}>{query.trim()}</strong>
															<span style={{ fontSize: 12 }}>Send invite link</span>
														</span>
							...
							<SettingsCard
								title="Invite Members"
								subtitle="Only admins and owners can invite new members."
							>
								<p style={{ margin: 0, fontSize: 13, color: "var(--text-secondary)" }}>Ask a workspace admin to send invitations.</p>
							</SettingsCard>
						)}
			...
			{activeTab === "danger" && (
				<SettingsCard
					title="Danger Zone"
					subtitle="Deleting this workspace removes boards, tasks, chat messages, whiteboard snapshots, and activity history."
					danger
				>
					<label
						style={{
