		...
		<section className="rounded-2xl border border-ghost-white-200 bg-white/90 p-6 shadow-sm">
			<div className="flex flex-wrap items-center justify-between gap-4">
				<div>
					...
					<p className="mt-1 text-sm text-jet-black-500">Drag and drop will be added in Phase 3.</p>
				</div>
				<Link
			...
			<div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
				{board?.columns?.map((column) => (
					<div
						key={column._id}
						className="rounded-2xl border border-ghost-white-200 bg-ghost-white-100/70 p-3"
					>
						...
						<div className="mt-3 space-y-3">
							{column.tasks?.length ? (
								column.tasks.map((task) => (
									<div
										key={task._id}
										className="rounded-xl border border-ghost-white-200 bg-white p-3 shadow-sm"
									>
										<h4 className="text-sm font-semibold text-jet-black-900">{task.title}</h4>
										{task.description && <p className="mt-1 text-xs text-jet-black-500">{task.description}</p>}
