	...
	if (status === "loading") {
		return (
			<section className="rounded-2xl border border-ghost-white-200 bg-white/90 p-6 shadow-sm">
				<p className="text-sm text-jet-black-500">Loading workspace...</p>
			</section>
		);
	}
			...
			<div className="mt-6 grid gap-4 md:grid-cols-2">
				<div className="rounded-xl border border-ghost-white-200 bg-ghost-white-100/70 p-4">
					...
					{workspace?.members?.length ? (
						<ul className="mt-3 space-y-2 text-sm">
							{workspace.members.map((member) => (
								...
								>
									...
									<span className="rounded-full bg-ghost-white-200 px-2.5 py-0.5 text-xs font-semibold text-jet-black-700">
										{member.role}
									</span>
								</li>
							))}
						...
						<p className="mt-3 text-sm text-jet-black-500">No members yet.</p>
					)}
				</div>
				<div className="rounded-xl border border-ghost-white-200 bg-ghost-white-100/70 p-4">
					<h3 className="text-lg font-semibold text-jet-black-900">Boards</h3>
					{boards.length ? (
						<ul className="mt-3 space-y-2 text-sm">
							{boards.map((board) => (
								<li key={board._id}>
						...
						<p className="mt-3 text-sm text-jet-black-500">No boards yet.</p>
					)}
				</div>
			</div>
		</section>
